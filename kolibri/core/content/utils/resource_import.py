...
class ResourceImportManagerBase(JobProgressMixin, metaclass=ABCMeta):
    ...
    def do_channel_database_import(self):
        ...
        admin_imported_ids = list(
            ContentNode.objects.filter(
                channel_id=self.channel_id, available=True, admin_imported=True
            )
            .exclude(kind=content_kinds.TOPIC)
            .values_list("id", flat=True)
        )
        not_admin_imported_ids = list(
            ContentNode.objects.filter(
                channel_id=self.channel_id, available=True, admin_imported=False
            )
            .exclude(kind=content_kinds.TOPIC)
            .values_list("id", flat=True)
        )

        filetransfer = self.create_channel_database_transfer(dest)
                ...
                try:
                    ...
                    if import_ran:
                        if node_ids:
                            ...
                            annotation.update_content_metadata(
                                self.channel_id, node_ids=node_ids
                            )
    ...
    def prepare_for_import(self):
        ...
        # Calculate channel database size if we're importing it
        if self.import_channel_database:
...
class RemoteResourceImportManagerBase(ResourceImportManagerBase):
    ...
    def get_channel_database_size(self):
        ...
        url = paths.get_content_database_file_url(
            self.channel_id, baseurl=self.baseurl
        )
