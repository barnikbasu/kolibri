...
4. **Use theme tokens** - Leverage the design system's spacing tokens:

.. code-block:: html

  <div :style="{ color: $themeTokens.primary }">...</div>
...
.. note::
  This restriction only applies to **inline** styles (``style=""`` or ``:style=""``) because they bypass the RTLCSS build process. Regular CSS in ``<style>`` blocks is automatically processed and safe to use.

.. note::
  While you *could* write RTL-aware conditionals inline like ``:style="{ [isRtl ? 'marginRight' : 'marginLeft']: '8px' }"``, the linting rule will flag this to encourage best practices. Instead, use a computed property which keeps templates clean and makes the logic testable.
...
1. Run ``yarn run lint-frontend`` and fix any ``kolibri/vue-no-rtl-breaking-inline-styles`` errors
...
.. note::
  RTL support is currently broken when running the development server with hot reloading enabled (``yarn run devserver-hot``). Use the regular dev server for RTL testing.
...
You'll also need Django installed for the i18n utilities (this is typically already available in the Kolibri development environment).
...
Before translators can begin working on the strings in our application, they need to be uploaded to Crowdin. Translations are maintained in the `release` branch on Crowdin in the `Crowdin kolibri project <http://crowdin.com/project/kolibri>`__.
